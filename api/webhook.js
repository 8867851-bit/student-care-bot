...
// ===== THEME (FINAL) =====
const THEME = {
  bg: "#FBFAF8",        // warm white (ฐานของทุกอย่าง)
  card: "#FFFFFF",

  text: "#2A2A2A",      // soft black (ไม่แข็ง)
  sub: "#7A7A7A",

  primary: "#3A7D6B",   // 🌿 deep green (ปุ่มหลัก)
  accent: "#A8D5BA",    // 🌱 soft green (highlight)
};
  ...
  return {
    ...
    body: {
      ...
      contents: [
        ...
        softText("ขอบคุณที่เล่าให้ฟังนะ 🤍"),

        pause(),

        {
          type: "text",
          text: "มันกระทบกับคุณแค่ไหนในช่วงนี้",
          weight: "bold",
          size: "md",
          color: THEME.text
        },
...
function UI_q4() {
  return card([
    ...
    btn("มีผู้ใหญ่ / ครู", "step_3_q4_adult")
  ]);
}
// Q5
function UI_q5() {
...
// TRANSITION
function UI_transition_cinematic() {
  return card([
    ...
    messages: [
      {
        type: "flex",
        altText: "welcome",
        contents: UI_entry_main()
      },
      {
        type: "flex",
        altText: "info",
        contents: UI_entry_topping()
      }
    ]
  });
...
async function handlePostback(event) {
 ...
 if (data === "get_slots") {
  ...
  return replyFlex(event.replyToken, UI_slotsWeekly(slots));
}
...
/* // ================= PREMIUM THEME =================
const THEME = {
  primary: "#3A3A3A"      // softer than black
accent: "#E8B4B8"       // dusty pink (keep identity)
bg: "#FAF9F7"           // warm off-white (สำคัญมาก)
highlight: "#FFF1F2"    // soft pink glow
text: "#1A1A1A"
subtext: "#7A7A7A"
};
